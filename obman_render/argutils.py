...
def parse_args(parser):
    ...
    if '--' in all_args:
        print('Parsing python arguments !')
        script_json = all_args[all_args.index('--') + 1:]
        config_updates = json.load(script_json)
    return config_updates

def save_args(args, save_folder, opt_prefix='opt', verbose=True, check_git=True):
    opts = vars(args)
